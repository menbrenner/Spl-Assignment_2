...
public abstract class MicroService implements Runnable {
    ...
    private ConcurrentHashMap<Class<?> ,Callback<?>> MStable;
    ...
    protected final <T, E extends Event<T>> void subscribeEvent(Class<E> type, Callback<E> callback) {
        //store the callback that fits to this type of event in HashMap, in order to call it later, during the ren.
		MStable.put(type,callback);
    ...
    public final void run() {
    	Bus.register(this);
        initialize();
        //work until the duration ends.
        while (!terminated) {
        	try {	
                Message currentMessage=Bus.awaitMessage(this);
                Callback toBeCalled = MStable.get(currentMessage.getClass());
                toBeCalled.call(currentMessage);
        	}catch(Exception e) {
        	    e.printStackTrace();
            }
        }
        //unregister to message bus.
    	Bus.unregister(this);
    }
